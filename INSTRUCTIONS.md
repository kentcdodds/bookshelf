...
user from that route to `/discover` automatically. React Router (currently)
supports a `<Redirect />` component, but it's technically incorrect to have the
client manage redirects for known routes. The server should be configured to
handle those.
